...
def check(source,pptx):
    ...
    for u in source['units']:
        t=normalize(u['text']);hits=[i+1 for i,p in enumerate(pages) if t in p]
    ...
    return {'passed':not missing and not order and not traditional,'sourceUnits':len(source['units']),'slides':len(pages),'missing':missing,'outOfOrder':order,'traditionalJian':traditional,'mapping':mapping,'scope':'PPTX vs source transcription only; image accuracy and visual quality need independent review'}
